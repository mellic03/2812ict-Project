...
class HandRenderer:
    ...
    def __init__(self, configpath) -> None:
        ...
        self.side_dir = glm.vec3(1.0, 0.0, 0.0)


    def is_grabbing(self, whandLms) -> bool:

        # First, compute lookAt matrix
        lookat_palm = glm.lookAt(glm.vec3(0.0), self.palm_dir, glm.vec3(0.0, 1.0, 0.0))
        lookat = lookat_palm


        wlandmarks = whandLms.landmark
        wlms = [glm.vec3(0.0)] * self.wlms.shape[0]

        for i in range(0, len(wlms)):
            vec = glm.vec4(wlandmarks[i].x, wlandmarks[i].y, wlandmarks[i].z, 1.0)
            wlms[i] = glm.vec3(lookat * vec)



        for joint_list in JOINT_LISTS:
            for i in range(0, len(joint_list)-1):
                pos_u = glm.vec3(wlms[joint_list[i+0]])
                pos_v = glm.vec3(wlms[joint_list[i+1]])
        ...
        x4  = math.fabs(wlms[ 4].x)
        x5  = math.fabs(wlms[ 5].x)
        x17 = math.fabs(wlms[17].x)

        xmax = max(x5, x17)
        xmin = min(x5, x17)

        y0  = math.fabs(wlms[ 0].y - wlms[0].y) * defs.IMG_H
        y5  = math.fabs(wlms[ 5].y - wlms[0].y) * defs.IMG_H
        y8  = math.fabs(wlms[ 8].y - wlms[0].y) * defs.IMG_H
        y9  = math.fabs(wlms[ 9].y - wlms[0].y) * defs.IMG_H
        y12 = math.fabs(wlms[12].y - wlms[0].y) * defs.IMG_H
        y13 = math.fabs(wlms[13].y - wlms[0].y) * defs.IMG_H
        y16 = math.fabs(wlms[16].y - wlms[0].y) * defs.IMG_H
        y17 = math.fabs(wlms[17].y - wlms[0].y) * defs.IMG_H
        y20 = math.fabs(wlms[20].y - wlms[0].y) * defs.IMG_H
    ...
    def __preprocess_vertices(self, handLms, whandLms) -> None:
        ...
        self.wlms = geom.lmarks_to_np(whandLms.landmark, self.wlms, 1)
        ...
        self.wrist = glm.vec3(
            glm.lerp(XYSCALE * self.lms[0][0], self.wrist[0], 0.8),
            glm.lerp(XYSCALE * self.lms[0][1], self.wrist[1], 0.8),
            glm.lerp(glm.clamp(MAX_DEPTH + depth_m, MAX_DEPTH, MIN_DEPTH), self.wrist[2], 0.8)
        )

        # self.wrist = glm.vec3(
        #     XYSCALE * self.lms[0][0],
        #     XYSCALE * self.lms[0][1],
        #     glm.clamp(MAX_DEPTH + depth_m, MAX_DEPTH, MIN_DEPTH)
        # )

        # self.delta_pos = self.last_pos - self.wrist
        # self.last_pos  = self.wrist


        # Add wrist position to all other positions
        # --------------------------------------------------------------------------------------
        for i in range(0, len(self.wlms)):
            self.wlms[i][0] += self.wrist.x
            self.wlms[i][1] += self.wrist.y
            self.wlms[i][2] += self.wrist.z
        # --------------------------------------------------------------------------------------
    ...
    def __draw(self, handDetector, handLms, whandLms, cam) -> None:
        ...
        # Hand orientation visualisation
        # --------------------------------------------------------------------------------------
        pos_0   = glm.vec3(self.wlms[0])
        pos_5   = glm.vec3(self.wlms[5])
        pos_17  = glm.vec3(self.wlms[17])
        self.palm_dir = glm.normalize(glm.cross(pos_5 - pos_0, pos_17 - pos_0))
        self.side_dir = glm.normalize(pos_17 - pos_5)
        ...
        pos_rot = glm.inverse(glm.lookAt(pos_0, pos_0+self.side_dir, glm.vec3(0.0, 1.0, 0.0)))
        pos_trans = glm.translate(pos_0)
        transform = pos_trans * pos_rot * pos_scale
